...
B2Vector3D BFieldComponent3d::calculate(const B2Vector3D& point) const
{
  ...
  auto getPhiIndexWeight = [this](double y, double x, double & wphi) -> unsigned int {
    ...
    ////////////////////Comments////////////////////
    //B2INFO("phi calculated(rad) = "<<phi);
    const double pi = 3.14159265359;
    B2INFO("phi calculated(deg) = "<< (phi*180)/pi );
    ////////////////////Comments////////////////////
    ...
    //Rotate point(position-vector) from BASF2 coordinates to ARIMOTO-FILE coordinates
    B2INFO("point = "<<point.x()<<"\t"<<point.y()<<"\t"<<point.z());
    //B2Vector3D point_prime( point.x(), point.y(), point.z() );
    //TVector3 point_prime( point.x(), point.y(), point.z() );
    B2Vector3D point_prime = point + B2Vector3D(0.0, 0.1, 0.0);
    //B2Vector3D point_prime = point;
    //const double conversion_angle = TMath::Pi()+0.0415; //hard coded for now
   //B2INFO("conversion_angle = "<<conversion_angle);
    //point_prime.RotateY( conversion_angle );
    const double cc = -0.99913899858; const double ss = -0.04148808879;
    B2Vector3D tmp = point_prime; 
    point_prime.SetZ( cc*tmp.z() - ss*tmp.x() ); point_prime.SetX( ss*tmp.z() + cc*tmp.x() );
    B2INFO("point-prime = "<<point_prime.x()<<"\t"<<point_prime.y()<<"\t"<<point_prime.z());
  ...
  double z = point_prime.z();
     B2INFO("calculated z = "<< z);
  double r2 = point_prime.Perp2();
    B2INFO("calculated r = "<< sqrt(r2) );

  // Check if the point lies inside the magnetic field boundaries
  if (z < m_mapRegionZ[0] || z > m_mapRegionZ[1]) {
    B2INFO("Z-criterion not satisfied");
    return B;}


  // Check if the point lies inside the magnetic field boundaries
  if (r2 < m_mapRegionR[0]*m_mapRegionR[0] || r2 >= m_mapRegionR[1]*m_mapRegionR[1]) {
    B2INFO("r-criterion not satisfied");
    return B;}

  // Check if the point lies in the exclude region
  if (m_exRegion && (z >= m_exRegionZ[0]) && (z < m_exRegionZ[1]) &&
      (r2 >= m_exRegionR[0]*m_exRegionR[0]) && (r2 < m_exRegionR[1]*m_exRegionR[1])) {
    B2INFO("point lies in excluded region");
    return B;}

  // Calculate the lower index of the point in the Z grid
  // Note that z coordinate is inverted to match ANSYS frame
  double wz = ( z - m_mapRegionZ[0]) * m_igridPitch[2];
  ...
  if (r2 > 0) {
    ...
    //double ay = std::abs(point_prime.y());
    double wphi;
    //unsigned int iphi = getPhiIndexWeight(ay, point.x(), wphi);
    unsigned int iphi = getPhiIndexWeight(point_prime.y(), point_prime.x(), wphi);

    //B2INFO("x, y = "<<point.x() <<" "<<point.y() );
    //B2INFO("iphi, wphi, m_igridPitch[1] = "<<iphi<<" "<<wphi<<" "<<m_igridPitch[1]);
    //B2INFO("iphi = "<< iphi);
    //B2INFO("wphi = "<< wphi);


    //B2INFO("r calculated = "<< r);
    //B2INFO("ir = "<< ir);
    //B2INFO("wr = "<< wr);
    //B2INFO("z recieved = "<< z );
    //B2INFO("iz = "<< iz);
    ...
    //Rotate B-vector from ARIMOTO-FILE coordinates back to BASF2 coordinates
    //B.RotateY( -1.0 * conversion_angle );
    tmp = B;
    B.SetZ( cc*tmp.z() + ss*tmp.x() ); B.SetX( (-ss*tmp.z()) + cc*tmp.x() );
...
/*
  B2INFO("B[j000] = "<< B[j000].x()<<"\t"<<B[j000].y()<<"\t"<<B[j000].z()<<"\t weight = "<<w00*wz0);
  B2INFO("B[j001] = "<< B[j001].x()<<"\t"<<B[j001].y()<<"\t"<<B[j001].z()<<"\t weight = "<<w01*wz0);
  B2INFO("B[j010] = "<< B[j010].x()<<"\t"<<B[j010].y()<<"\t"<<B[j010].z()<<"\t weight = "<<w10*wz0);
  B2INFO("B[j011] = "<< B[j011].x()<<"\t"<<B[j011].y()<<"\t"<<B[j011].z()<<"\t weight = "<<w11*wz0);
  B2INFO("B[j100] = "<< B[j100].x()<<"\t"<<B[j100].y()<<"\t"<<B[j100].z()<<"\t weight = "<<w00*wz1);
  B2INFO("B[j101] = "<< B[j101].x()<<"\t"<<B[j101].y()<<"\t"<<B[j101].z()<<"\t weight = "<<w01*wz1);
  B2INFO("B[j110] = "<< B[j110].x()<<"\t"<<B[j110].y()<<"\t"<<B[j110].z()<<"\t weight = "<<w10*wz1);
  B2INFO("B[j111] = "<< B[j111].x()<<"\t"<<B[j111].y()<<"\t"<<B[j111].z()<<"\t weight = "<<w11*wz1);
*/
