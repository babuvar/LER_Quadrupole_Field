...
for i in range(24):
    print("i=%s, phivals[-i]=%s"%(i,phivals[-i]))
...
for name in names:
    ...
    for i_z in range(7):
        ...
        for i in range(12):
            ...
            pl.plot(rvals, B_r[23-i,:,i_z], marker='o', color='red', label='Belle-2 Solenoid+Quad Field')
            pl.plot(rvals, B_r_wo[23-i,:,i_z], marker='o', color='green', label='Belle-2 Solenoid Field')
        ...
        for i in range(12):
            pl.subplot(4,3,i+1);
            pl.plot(100*rad, Br[:,i], marker='o', label='Arimoto-san field'); pl.title('z=%s m, phi = %s deg'%(zval, phi[i]), color='blue')
            pl.plot(rvals, B_r[23-i,:,i_z] - B_r_wo[23-i,:,i_z], marker='o', color='orange', label='Belle-2 Quad Field Alone')
            pl.ylabel('B_r (Tesla)'); pl.xlabel('radius (cm)');
            pl.legend(loc='lower right')
